...
def load_scene_and_run_gen(scene):
    ...
    try:
        ...
        print("estimated position: ", result_particle.position[0], result_particle.position[1], result_particle.theta)
        print("ground truth: ", odometry[-1, 0], odometry[-1, 1], odometry[-1, 2])
    
    print(f"Error is {error:.6f}")
    return error

def optimal(test_idx):
...
if __name__ == "__main__":
    ...
    if not args.test:
        ...
        print("result: ", result_particle.position, result_particle.theta)
        print(f"Error is {error:.6f}")