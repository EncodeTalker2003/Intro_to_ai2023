...
def plot_S(S):
    ...
    print(S)
    sum_all = np.sum(S ** 2)
    E = np.array([np.sum(S[:(i+1)] ** 2) / sum_all for i in range(S.size)])
    return E
...
def matmul_U_S_V(U, S, V):
    ...
    diag_s = np.diag(S)
    new_mat = np.dot(np.dot(U, diag_s), V)
    return new_mat
...
def task2(U, S, V):
    ...
    '''
    prec = plot_S(S)
    ind = np.array([i for i in range(S.size)])
    plt.figure()
    plt.plot(ind,prec)
    plt.show()
