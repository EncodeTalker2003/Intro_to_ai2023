...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    not_visited = list(set(corners) - set(Visited_Corners))
    not_visited_len = len(not_visited)
    if not_visited_len == 0:
        return 0
    p = list(permutations(not_visited))
    for per in p:
        tmp = util.manhattanDistance(node, per[0])
        for i in range(not_visited_len - 1):
            tmp += util.manhattanDistance(per[i], per[i + 1])
        h_sum = min(h_sum, tmp)
    return h_sum
