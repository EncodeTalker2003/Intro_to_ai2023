...
class Linear(Node):
    ...
    def backcal(self, grad):
        weight, bias = self.params
        X = self.cache[-1]
        self.grad.append(np.matmul(X.T, grad))
        self.grad.append(np.sum(grad, axis=0))
        return np.matmul(grad, weight.T)
...
class LogSoftmax(Node):
    ...
    def backcal(self, grad):
        return grad - np.exp(self.cache[-1]) * grad.sum(axis=self.dim, keepdims=True)
...
class CrossEntropyLoss(Node):
    ...
    def cal(self, X):
        ...
        y = self.y
        return - np.sum(
            np.take_along_axis(np.log(X + self.EPS), np.expand_dims(y, axis=-1), axis=-1))
